...
function main() {
  ...
  fs.writeFileSync(path.join(ff, "ff-background.js"), [
    ...
    '  const out = [];',
    '  out.push(await globalThis.runPinProbe("firefox-mv3-event-page"));',
    '  try {',
    '    const w = new Worker("worker.js");',
    '    const fromWorker = await new Promise((res) => {',
    '      w.onmessage = (e) => res(e.data);',
    '      w.onerror = (e) => res([{ context: "firefox-event-page-dedicated-worker", fatal: String(e.message) }]);',
    '      w.postMessage({ type: "run" });',
    '      setTimeout(() => res([{ context: "firefox-event-page-dedicated-worker", fatal: "timeout" }]), 180000);',
    '    });',
    '    w.terminate();',
    '    for (const r of fromWorker) out.push(r);',
    '  } catch (e) {',
    '    out.push({ context: "firefox-event-page-dedicated-worker", fatal: String(e).slice(0, 200) });',
    '  }',
    '  try {',
    '    await fetch("' + COLLECTOR + '/results", { method: "POST",',
    '      headers: { "content-type": "application/json", "x-pratibimb-probe": "report" },',
  ...
  fs.writeFileSync(path.join(ff, "worker.js"), [
    ...
    '  self.postMessage([await globalThis.runPinProbe("firefox-event-page-dedicated-worker")]);',
    '};'
  ].join("\n"));
