 ...
 *
 *   BACKEND=wasm   node .../run-qg03b-chrome.mjs
 *   BACKEND=webgpu node .../run-qg03b-chrome.mjs
 *
...
async function oneRun({ headless, run }) {
  const udd = mkdtempSync(join(tmpdir(), "pratibimb-qg03b-"));
  ...
  try {
    ...
    out.result = await sw
      .evaluate((c) => globalThis.__qg03b_run(c), { backend: BACKEND, warmRuns: WARM })
