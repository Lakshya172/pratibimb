/**
 * QG-03b-2a Chromium runner. CHROMIUM ONLY — the question is Chromium's own encoder,
 * so there is deliberately no Firefox counterpart to this file.
 *
 *   BACKEND=wasm   node .../run-qg03b2a-chrome.mjs
 *   BACKEND=webgpu node .../run-qg03b2a-chrome.mjs
 *
...
async function oneRun({ headless, run }) {
  ...
  try {
    ...
    out.result = await sw
      .evaluate((c) => globalThis.__qg03b2a_run(c), { backend: BACKEND, warmRuns: WARM })
...
const path = join(HERE, "..", "logs", `results-qg03b2a-chromium-${BACKEND}-${LABEL}.json`);
