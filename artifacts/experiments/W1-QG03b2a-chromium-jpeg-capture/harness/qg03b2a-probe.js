/**
 * QG-03b-2a — does the browser decode ITS OWN capture the way the reference decoder does?
 *
 * QG-03b-2 answered the same-shaped question for Pillow-encoded JPEGs and got bitwise
 * agreement on every format. That result carried a caveat that could not be removed from
 * inside it: Pillow's encoder and Chromium's decoder are both libjpeg-turbo, so agreement
 * was partly structural. The files here were written by Chromium's own capture encoder and
 * the reference decoder meets them as a stranger.
 *
 * They also carry encoder choices the synthetic fixtures never had — 4:2:0 chroma
 * subsampling and a 456-byte embedded sRGB ICC profile — and both are exactly the kind of
 * thing that separates two decoders.
 *
 * ─────────────────────────────────────────────────────────────────────────────────────
 * WHAT IS AND IS NOT BEING COMPARED
 *
 *     the SAME CAPTURED BYTES, decoded here, against the SAME CAPTURED BYTES decoded by
 ...
 * are two different images of one paint and their difference is compression, measured
 * separately as `compressionSensitivity` and never allowed to become a verdict.
 *
 * The encoded file's SHA-256 is verified HERE, in the browser, before anything else runs,
 * so "both sides decoded the same file" is a measurement and not an assumption. The magic
 * bytes are checked against the MIME the capture declared for the same reason: a file that
 * says JPEG and starts with a PNG signature would make every number below meaningless.
 *
 * ─────────────────────────────────────────────────────────────────────────────────────
 * THREE DECODE VARIANTS, BECAUSE "THE BROWSER DECODED IT" IS NOT ONE BEHAVIOUR
 *
 *   default            createImageBitmap with no options
 *   no-colorspace      colorSpaceConversion: "none" — isolates COLOUR MANAGEMENT, which is
 *                      load-bearing here in a way it was not in QG-03b-2: these files carry
 *                      an ICC profile, Pillow does not apply one, and on the widest-gamut
 *                      fixture applying it moves a pixel by one level. If default and
 *                      no-colorspace agree, the browser did not apply it either.
 *   no-premultiply     premultiplyAlpha: "none" — isolates the canvas ALPHA round trip.
 *
 ...
 */
globalThis.runQg03b2aProbe = async function runQg03b2aProbe(contextName, opts) {
  opts = opts || {};
  const F = globalThis.QG03B2A_FIXTURES;
  const base = opts.base || "";
  const backend = opts.backend || "wasm";

  const out = {
    context: contextName,
    probe: "qg03b2a-real-chromium-jpeg-capture",
  ...
  // The MIME comes from the fixture itself: it is the one the browser's own data URL
  // declared at capture time, carried through the manifest. Hard-coding a table here would
  // let the probe assert a type the capture never claimed.
  const mimeOf = (f) => f.mime;
  ...
  /**
   * Count real GPU submissions, so a "webgpu" label is an OBSERVATION.
   *
   * QG-03 established this discriminator and the reason for it: ORT will fall back to
   * another execution provider without raising, so a session configured for WebGPU that
   * never submits a command buffer ran on the CPU. The hook is installed before any session
   * exists. It is only a label check — the decode and preprocessing measured below are
   * CPU-side and identical on both backends — but an unverified label is still a claim.
   */
  const gpu = { submits: 0, hookError: null };
  try {
    if (typeof GPUQueue === "function") {
      ...
      gpu.hookError = "GPUQueue is not a constructor in this context";
    }
  } catch (e) {
    gpu.hookError = String((e && e.message) || e).slice(0, 200);
  }
  ...
  try {
    ...
    for (const encName of ["capture-png", "capture-jpeg"]) {
      const f = F.fixtures.find((x) => x.encoding === encName && x.page === "text-heavy" && x.display === "headful");
      if (!f) continue;
    ...
    for (const f of F.fixtures) {
      const rec = { source: f.source, page: f.page, display: f.display, encoding: f.encoding, lossless: f.lossless, variants: {} };
      try {
        const bytes = b64ToBytes(F.blobs[f.file]);
        ...
        // Independent of the manifest: does the file actually START like the type it claims?
        // The capture declared a MIME in its data URL; a mismatch here would mean the bytes
        // and the label parted company somewhere between the browser and this probe.
        const sig = Array.from(bytes.slice(0, 4)).map((b) => b.toString(16).padStart(2, "0")).join(" ");
        rec.magic = sig;
        rec.magicMatchesDeclaredMime =
          f.mime === "image/png" ? sig === "89 50 4e 47" : f.mime === "image/jpeg" ? sig.startsWith("ff d8 ff") : null;
        ...
        for (const variant of ["default", "no-colorspace", "no-premultiply"]) {
          ...
          try {
            ...
            v.conformant = v.refusedNotOpaque
              ...
              : rec.magicMatchesDeclaredMime !== true
                ? false // the bytes are not the type the capture said they were
                : f.lossless
    ...
    if (F.model && typeof ort !== "undefined") try {
      ...
      for (const f of F.fixtures) {
        ...
        const rec = { source: f.source, page: f.page, display: f.display, encoding: f.encoding };
        try {
          const dec = await decodeNative(b64ToBytes(F.blobs[f.file]), mimeOf(f), "default");
      ...
      // Recorded whatever the backend was. On wasm this is expected to be zero, and a
      // NON-zero count there would be just as interesting as a zero count on webgpu.
      out.gpu = { submits: gpu.submits, hookError: gpu.hookError, observedBackend: gpu.submits > 0 ? "webgpu" : "cpu" };
      out.backendLabelVerified = backend === "webgpu" ? gpu.submits > 0 : gpu.submits === 0;
    } catch (e) {
