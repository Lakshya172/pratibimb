...
const probe = fs.readFileSync(path.join(__dirname, "s04-probe.js"), "utf8")
...
fs.writeFileSync(path.join(OUT_CHROME, "manifest.json"), JSON.stringify({
  manifest_version: 3,
  name: "PratiBimb S-04 ORT session lifecycle probe (Chrome)",
  version: "1.0.0",
  ...
  host_permissions: ["http://127.0.0.1:8907/*"],
...
fs.writeFileSync(path.join(OUT_CHROME, "offscreen.js"), [
  ...
  '  if (!msg || msg.type !== "s04-go") return;',
  '  (async () => {',
  '    const base = chrome.runtime.getURL("");',
  '    const results = [];',
  '    for (const cfg of msg.configs) {',
  '      results.push(await globalThis.runS04Probe("chrome-offscreen-document", Object.assign({}, cfg, { wasmPaths: base })));',
  '    }',
  ...
  '    chrome.runtime.sendMessage({ type: "s04-offscreen-results", results: results });',
  '  })();',
  '});',
].join("\n"));
...
fs.writeFileSync(path.join(OUT_CHROME, "worker.js"), [
  ...
  '    results.push(await globalThis.runS04Probe("chrome-offscreen-dedicated-worker", Object.assign({}, cfg, { wasmPaths: base })));',
  '  }',
...
fs.writeFileSync(path.join(OUT_CHROME, "background.js"), [
  ...
  '  await chrome.offscreen.createDocument({ url: "offscreen.html", reasons: ["WORKERS"], justification: "S-04 ORT session lifecycle probe" });',
  '}',
  '',
  'globalThis.__s04_run = async function (configs) {',
  '  const all = [];',
  '  const base = chrome.runtime.getURL("");',
  '  for (const cfg of configs) {',
  '    all.push(await globalThis.runS04Probe("chrome-mv3-service-worker", Object.assign({}, cfg, { wasmPaths: base })));',
  '  }',
  ...
  '      if (m && m.type === "s04-offscreen-results") { clearTimeout(t); chrome.runtime.onMessage.removeListener(h); resolve(m.results); }',
  '    });',
  '    chrome.runtime.sendMessage({ type: "s04-go", configs: configs }).catch(() => {});',
  '  });',
...
fs.writeFileSync(path.join(OUT_FIREFOX, "manifest.json"), JSON.stringify({
  manifest_version: 3,
  name: "PratiBimb S-04 ORT session lifecycle probe (Firefox)",
  version: "1.0.0",
  description: "Throwaway spike harness. Creates and runs a real ONNX Runtime Web session in a Firefox MV3 event page.",
  browser_specific_settings: { gecko: { id: "pratibimb-s04@example.invalid", strict_min_version: "128.0" } },
...
fs.writeFileSync(path.join(OUT_FIREFOX, "ff-background.js"), [
  ...
  '  const results = [];',
  '  for (const cfg of CONFIGS) {',
  '    try { results.push(await globalThis.runS04Probe("firefox-mv3-event-page", Object.assign({}, cfg, { wasmPaths: base }))); }',
