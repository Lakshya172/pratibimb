"""
QG-03a-C3 — decode the rendered PNGs to raw RGBA with Pillow.

  python artifacts/experiments/W1-QG03a-C3-label-raster-generalisation/harness/decode-c3-png.py
...
on these RGBA bytes in Node, which is the stage C3 is actually about.
...
def main():
    ...
    if not manifests:
        sys.stderr.write(f"no manifests under {GEN} - run render-c3.mjs first\n")
        sys.exit(2)
    total = 0
