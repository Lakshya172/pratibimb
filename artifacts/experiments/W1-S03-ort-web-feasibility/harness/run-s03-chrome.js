...
async function oneRun({ headless, run }, received) {
  ...
  try {
    ...
    out.contexts = await sw.evaluate((c) => globalThis.__s03_run(c), CONFIGS)
...
(async () => {
  ...
  const out = { experiment: "W1-S03-ort-web", browser: "chrome", startedAt: new Date().toISOString(),
  ...
  fs.writeFileSync(path.join(__dirname, `results-chrome-${PLATFORM}.json`), JSON.stringify(out, null, 2));
  server.close();
  process.stderr.write(`\nwrote results-chrome-${PLATFORM}.json\n`);
  process.exit(0);
})();
