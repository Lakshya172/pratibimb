...
(async () => {
  ...
  const out = {
    experiment: "W1-S03-ort-web", browser: "firefox",
  ...
  fs.writeFileSync(path.join(__dirname, `results-firefox-${process.env.PLATFORM_LABEL || "unknown"}.json`), JSON.stringify(out, null, 2));
  server.close();
