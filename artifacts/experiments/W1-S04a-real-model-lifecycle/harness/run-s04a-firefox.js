...
(async () => {
  ...
  const out = {
    experiment: "W1-S04a-real-model-lifecycle", browser: "firefox",
  ...
  fs.writeFileSync(path.join(__dirname, `results-s04a-firefox-${process.env.PLATFORM_LABEL || "unknown"}.json`), JSON.stringify(out, null, 2));
  server.close();
  process.stderr.write("\nwrote results-s04a-firefox-" + (process.env.PLATFORM_LABEL || "unknown") + ".json\n");
  process.exit(0);
})();
