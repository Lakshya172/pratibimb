...
const CONFIGS = process.env.COLD_START_BACKEND
  ? [{ backend: process.env.COLD_START_BACKEND, numThreads: 1, cycles: 5 }]
  : [
      { backend: "wasm", numThreads: 1, cycles: 5 },
      { backend: "webgpu", cycles: 5 },
    ];
...
async function oneRun({ headless, run }, received) {
  ...
  try {
    ...
    out.contexts = await sw.evaluate((c) => globalThis.__s04a_run(c), CONFIGS)
...
(async () => {
  ...
  const out = { experiment: "W1-S04a-real-model-lifecycle", browser: "chrome", startedAt: new Date().toISOString(),
  ...
  fs.writeFileSync(path.join(__dirname, `results-s04a-chrome-${PLATFORM}.json`), JSON.stringify(out, null, 2));
  server.close();
  process.stderr.write(`\nwrote results-s04a-chrome-${PLATFORM}.json\n`);
  process.exit(0);
})();
