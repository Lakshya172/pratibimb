 ...
 *
 *   BACKEND=wasm   node .../run-qg03-chrome.mjs
 *   BACKEND=webgpu node .../run-qg03-chrome.mjs
 *
...
async function oneRun({ headless, run }) {
  const udd = mkdtempSync(join(tmpdir(), "pratibimb-qg03-"));
  ...
  try {
    ...
    out.result = await sw
      .evaluate((c) => globalThis.__qg03_run(c), { backend: BACKEND, warmRuns: WARM })
...
const path = join(HERE, "..", "logs", `results-chromium-${BACKEND}-${LABEL}.json`);
