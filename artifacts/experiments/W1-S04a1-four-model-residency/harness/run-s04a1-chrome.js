...
async function oneRun({ headless, run }, received) {
  ...
  try {
    ...
    out.contexts = await sw.evaluate((c) => globalThis.__s04a1_run(c), CONFIGS)
...
(async () => {
  ...
  const out = { experiment: "W1-S04a1-four-model-residency", browser: "chrome", startedAt: new Date().toISOString(),
  ...
  fs.writeFileSync(path.join(__dirname, `results-s04a1-chrome-${PLATFORM}.json`), JSON.stringify(out, null, 2));
  server.close();
  process.stderr.write(`\nwrote results-s04a1-chrome-${PLATFORM}.json\n`);
  process.exit(0);
})();
