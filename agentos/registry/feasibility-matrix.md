...
| QG-03a-B3-2 | The same four cells on an **NVIDIA-backed** browser | **`NOT MEASURED / OPTIONAL COVERAGE`.** Workstation 1 has an RTX 5050, but Chrome selected the Intel `gen-12lp` adapter for WebGPU in the extension realm, so the discrete GPU was never exercised by B3-1. Forcing it is a **distinct cell**, not a re-run of B3-1. No NVIDIA evidence exists, none was simulated or inferred, and its absence is **not** recorded as a failure. **Update 2026-09-12 (owner decision, ronitsaha11):** the governing feasibility axis is **model × browser × backend** — the dossier defines **no GPU-vendor axis** — so an NVIDIA-backed cell is **not a mandatory acceptance cell** for QG-03a-B, and B3-1's Intel `gen-12lp` WebGPU cell **stands as the required WebGPU evidence**. NVIDIA coverage is **optional**: it affects **coverage and labelling only**, it does **not** invalidate B3-1 (which remains `PASS`), and **no NVIDIA compatibility or performance claim may be made** while it is unmeasured. This is **not** a retroactive mandatory gate; GPU-vendor selection stays tracked as an open coverage item under **S-01a**. | — |
| QG-03a-C3 | Does the §6 label/raster mismatch generalise — does it break coordinates at capture sizes OFF the training grid? | **MEASURED 2026-09-12 (workstation 2) — `INCONCLUSIVE / DECISION REQUIRED`. `QG-03a-C` stays `CONDITIONAL`; nothing was retrained.** Six pre-registered cells x 20 fresh synthetic samples (120 in total, none dropped) through the **shipped** path — the exact artifact `ba6d9e93…`, ORT Web 1.29.0 on the deterministic WASM/Node backend, the frozen evaluator, the 0.55 operating point, recall read against the per-cell CLIPPED ceiling (0.926-0.962). Four capture sizes off the training grid (1264x800, 2880x1443 at DPR 1.5, 1920x1080, 2560x1600) against two training-grid controls (960x640, 1024x640). **Why INCONCLUSIVE is not a near-miss:** the pre-registered design **cannot attribute what it measured**, because capture size moves the label/raster offset (<= **1.33 CSS px**) and the object scale in model space (**1.50 to 4.00 CSS px per model px**) together. The degradation tracks scale, monotonically, and the decisive internal control is **a2 versus a3** — different sizes, different DPR, different offsets, the **same 3.00 CSS px per model px**, near-identical scores (mAP@0.5 0.0608 vs 0.0617). **What it does establish:** at an off-grid but training-like scale the learned nuisance mapping holds — **1264x800** (the QG-03b-2a production capture size) scores mAP@0.5 **0.6647** and grounding **0.6396**, better than both controls. Also recorded as not working: `minMatchedIou` and `worstMatchedDispCss` are computed over pairs the matcher already gated at IoU >= 0.5, so the approved criterion's *"IoU stays well above 0.5"* clause **could not be evaluated**; no replacement statistic was invented. **No retrain, no relabel, no threshold change, no evaluator or CLIPPED change; `artifacts/datasets/` and `artifacts/gates/` never read or written; the consumed held-out split untouched.** Evidence: `artifacts/experiments/W1-QG03a-C3-label-raster-generalisation/`. | QG-03a-C |
| QG-03a-C3b | Isolate the inverse-letterbox convention: is the label/raster convention the CAUSE of C3's degradation? | **MEASURED 2026-09-12 (workstation 2) — `NOT ATTRIBUTED TO LETTERBOX CONVENTION`. No inference was run and no weights were loaded.** C3's **retained raw outputs** (120 samples, 36 MB, every `.f32` digest re-verified against the C3 logs) were decoded **once** and then projected **two ways**: the production **continuous** inverse (`computeLetterbox`, uniform scale, real pads — the space the labels were written in) against a **raster-consistent** inverse (`rasterLetterbox` integers, per-axis scales `capW/resizedW`, `capH/resizedH` — the change §6 prescribes). Everything else held byte-identical: same outputs, one shared `decodeHeadOutput` and NMS, same detection set, same 0.25 floor and 0.55 point, same sealed datasets, same DOM ground truth, same frozen evaluator. The harness contains **no ORT import at all**. **Decisive observation:** the two most collapsed cells have an inversion delta of **exactly zero** — at 1920x1080 and 2560x1600 the capture scales to integer content (640x360, 640x400), so the two letterboxes **are the same transform** and both paths emit byte-identical boxes — yet they score mAP@0.5 **0.0617** and **0.0040** against controls at 0.599-0.661. A provably identical transform cannot cause a difference, and **no threshold was needed** to reach that. Where the conventions differ most (2880x1443 at DPR 1.5, delta **1.998 CSS px**, the bound across all 120 samples) switching the inverse moves mAP@0.5 by **+0.0011**, about 0.2% of that cell's -0.54 gap. **No gap closed in either view, and the scale pattern is untouched.** Incidental and explicitly **not** a recommendation: the raster inverse is marginally better where the two differ on real UI (1264x800: +0.0117 mAP, +0.0126 recall, 5 more matched) and marginally worse on 960x640 (-0.0003) — far too small to justify touching a frozen coordinate transform, which the contract warns would move every emitted detection. **Consequence:** the §6 mismatch is **exonerated as a cause** and stays **recorded technical debt, not a demonstrated production defect**; **retraining is not justified** by C3 or C3b, and none was performed. C3 stays **INCONCLUSIVE** — C3b removed a candidate cause, it did not create a pass. **What remains open is not C's subject:** the detector is **scale-fragile beyond ~2 CSS px per model px** — see QG-03a-C4. Evidence: `artifacts/experiments/W1-QG03a-C3b-letterbox-inverse-attribution/`. | QG-03a-C |
| QG-03a-C | Closure of the label<->raster geometry concern | **`RESOLVED / CONDITIONAL-RESOLVED` — APPROVED by ronitsaha11, 2026-09-12, by merging PR #49 and recording the decision below. The geometry concern is CLOSED as NO DEMONSTRATED PRODUCTION DEFECT. The mismatch itself is NOT fixed and must not be described as fixed.** The **geometry concern is recommended as RESOLVED — NO DEMONSTRATED PRODUCTION DEFECT**, on four independent measurements: the mismatch is reproduced and bounded (84/200, <= 0.681 model px, QG-03a-C), metric-neutral at worst case (mAP@0.5, recall, grounding all 1.000 on train and dev), correct end-to-end at production capture sizes (QG-03a-B2 and B3-1, worst displacement 0.0026 / 0.002893 CSS px), off-grid-safe at production-like scale (C3 cell 1264x800 beats both controls), and now **directly exonerated as a cause** by C3b on identical raw outputs. **It is NOT fixed, and must not be described as fixed.** The §6 label/raster mismatch remains in `tools/detector/data.py`, retained as **TECHNICAL DEBT / FUTURE TRAINING-REVISION ITEM** with the exact change already written in `preprocessing-contract.md` §6, to be bundled with the next training change (QG-03a-C1). **Why this is the owner's call and not a measurement gap:** no pre-registered criterion converts C from `CONDITIONAL` to `PASS` — C's CONDITIONAL is pinned to the outstanding debt, and the one remaining unknown (the effect on training **dynamics**) can only be measured by a retrain that is **not justified on its own**. Two options are on the record in `artifacts/experiments/W1-QG03a-C3b-letterbox-inverse-attribution/decision.md`: **close C as CONDITIONAL-resolved with §6 as standing debt**, or **hold C CONDITIONAL until a training revision happens anyway**. **Neither closes QG-03a**, which still carries B and the scale finding; the detector stays **UNADOPTED** at threshold **0.55**. **Update 2026-09-12 (owner decision, ronitsaha11) — Option 1 taken. `QG-03a-C` = `RESOLVED / CONDITIONAL-RESOLVED`.** On the recorded evidence: the label/raster mismatch **is real**; C3 was inconclusive only because capture scale and label geometry were **confounded**; C3b changed **only** the inverse letterbox convention on **identical raw outputs** and moved the result by at most **1.998 CSS px** and **+0.0117 mAP@0.5**, against a C3 degradation of **0.54-0.60 mAP**; and cells where the two transforms are **exactly identical** degrade just as much. The degradation is therefore **not attributable to the label/raster convention**, and **no production-visible coordinate failure has been demonstrated**. **What this decision does NOT say:** it does not call the mismatch fixed, it does not close QG-03a, it does not adopt the detector, it does not certify the detector as scale-robust, and it sets **no numeric acceptance threshold for C** — none was created. The §6 mismatch in `tools/detector/data.py` is **retained as TECHNICAL DEBT / FUTURE TRAINING-REVISION ITEM**, with the exact change still written in `preprocessing-contract.md` §6, to be bundled with a future coordinated training revision (QG-03a-C1). **Retraining is NOT justified by this issue alone.** The separate scale-generalisation concern stays **OPEN as QG-03a-C4** and is **not folded into C**. | detector adoption |
| QG-03a-C4 | Detector scale fragility — NOT a label/raster question | **`OPEN` — a detector SCALE-GENERALISATION concern requiring adoption / capability work, confirmed as separate from QG-03a-C by the owner decision of 2026-09-12. OBSERVED 2026-09-12 (workstation 2). Recorded because C3 measured it, and explicitly NOT attributed to the label/raster convention.** Across C3's six cells mAP@0.5 falls monotonically with CSS px per model px: **1.50 -> 0.599, 1.60 -> 0.661, 1.98 -> 0.665, 3.00 -> 0.062, 3.00 -> 0.061, 4.00 -> 0.004**, and C3b showed the pattern is **unchanged** by the inverse convention. Attribution is partial, not complete: a 14 CSS px control shrinking to ~3.5 model px falls below the stride-8 feature cell, which is consistent with the collapse but was not isolated by a scale-controlled experiment. **No production capture limit may be inferred from this, and none is stated here** — the numbers are synthetic, the cell list was chosen to vary the label offset rather than to bound a policy, and a bound would be a new acceptance threshold. Where it belongs: **adoption item 11 (acceptable detector metrics, currently FAIL)** as a scope condition on any future metric claim, **item 14** only as something a later review must see, and **capture policy** — where **no scale or capture-size bound exists today**: ADR-0002 is a **format** policy (PNG only) and the dossier's only capture constraint is a latency budget. Creating one is a **future adoption decision**, not a documentation change. Evidence: `artifacts/experiments/W1-QG03a-C3-label-raster-generalisation/`, `artifacts/experiments/W1-QG03a-C3b-letterbox-inverse-attribution/`. | detector adoption (item 11) |
